...
public class CSVtoMonsterSO
{
    ...
    public static void GenerateMonsterSO()
    {
        ...
        for (int i = 0; i < tableSize; i++)
        {
            ...
            monsterSO.baseHP = int.Parse(allLines[numberOfColumns * (i + 1) + 2]);
            ...
            monsterSO.baseATK = int.Parse(allLines[numberOfColumns * (i + 1) + 4]);
            monsterSO.baseDEF = int.Parse(allLines[numberOfColumns * (i + 1) + 5]);
            monsterSO.baseSpeed = int.Parse(allLines[numberOfColumns * (i + 1) + 6]);
            monsterSO.baseER = int.Parse(allLines[numberOfColumns * (i + 1) + 7]);
            monsterSO.baseEnergy = int.Parse(allLines[numberOfColumns * (i + 1) + 8]);
            monsterSO.baseHR = int.Parse(allLines[numberOfColumns * (i + 1) + 9]);
            monsterSO.baseCritRate = int.Parse(allLines[numberOfColumns * (i + 1) + 10]);
            monsterSO.baseCritDame = int.Parse(allLines[numberOfColumns * (i + 1) + 11]);
            if (!File.Exists($"{path}/{tempName}.asset"))
            {
